    ...
    async fn execute(&self, args: Vec<String>, ctx: CommandContext) -> Result<String> {
        ...
        match subcommand {
            ...
            "config" => {
                ...
                match op {
                    ...
                    "get" => {
                        let key = args
                            .get(3)
                            .context("Missing key: /providers config <id> get <key>")?;
                        let val = config
                            .read_config(key)
                            .await?
                            .unwrap_or_else(|| "(not set)".to_string());
                    ...
                    "set" => {
                        let key = args
                            .get(3)
                            .context("Missing key: /providers config <id> set <key> <value>")?;
                        let val = args
                            .get(4)
                            .context("Missing value: /providers config <id> set <key> <value>")?;
            ...
            _ => {
                ...
                anyhow::bail!(
                    "Unknown subcommand: {}. Use list, set, or config.",
                    subcommand
                )
            }
        }

...
impl Database {
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        if !has_kind {
            ...
            sqlx::query("ALTER TABLE memories ADD COLUMN kind TEXT NOT NULL DEFAULT 'long'").execute(&self.pool).await?;
            sqlx::query("ALTER TABLE memories ADD COLUMN brief TEXT NOT NULL DEFAULT ''").execute(&self.pool).await?;
            sqlx::query("ALTER TABLE memories ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''").execute(&self.pool).await?;
            // Copy content to brief for existing ones
            sqlx::query("UPDATE memories SET brief = SUBSTR(content, 1, 100)").execute(&self.pool).await?;
        }
...
impl minus_api::traits::MinusDatabase for Database {
    ...
    async fn get_messages(&self, chat_id: &str, limit: i64) -> Result<Vec<minus_api::Message>> {
        ...
        Ok(records.into_iter().map(|r| minus_api::Message {
            id: r.id,
            chat_id: r.chat_id,
            role: r.role,
            content: r.content,
    ...
    async fn log_audit(&self, id: &str, actor: &str, action: &str, target: Option<&str>, metadata: Option<&str>, created_at: &str) -> Result<()> {
        self.log_audit(id, actor, action, target, metadata, created_at).await
    }
    async fn tail_audit(&self, limit: i64) -> Result<Vec<minus_api::AuditEvent>> {
        let records = self.tail_audit(limit).await?;
        Ok(records.into_iter().map(|r| minus_api::AuditEvent {
            id: r.id,
            actor: r.actor,
            action: r.action,
            target: r.target,

...
async fn main() -> Result<()> {
                ...
                if let Some(val) = args.next() {
                    match val.to_lowercase().as_str() {
                        ...
                        "unix" => {
                            ...
                            #[cfg(not(unix))]
                            {
                                eprintln!("Unix sockets not supported on this OS");
                                std::process::exit(1);
                            }
                        }
                        _ => {
            ...
            match packet {
                ...
                CliPacket::ToolCall(p) => {
                    let _ = printer.print(format!(
                        "\x1b[1;36m[TOOL]\x1b[0m Calling: \x1b[33m{}\x1b[0m \x1b[90m({})\x1b[0m",
                        p.name, p.brief
                    ));
                }
    ...
    loop {
        ...
        let prompt = format!(
            "\x1b[1;34muser\x1b[0m (\x1b[1;32m{}\x1b[0m) \x1b[90m>\x1b[0m ",
            chat_id
        );
